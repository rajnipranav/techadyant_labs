...
const SITE = process.env.GSC_SITE || 'sc-domain:labs.techadyant.com';
...
function openBrowser(url) {
  try {
    if (process.platform === 'win32') spawn('cmd', ['/c', 'start', '', url], { shell: false, detached: true, stdio: 'ignore' }).unref();
...
async function authorize() {
  ...
  const challenge = verifier; // S256 needs hashing; 'plain' is acceptable for loopback
  const url = 'https://accounts.google.com/o/oauth2/v2/auth?' + new URLSearchParams({
    ...
    prompt: 'consent', code_challenge: challenge, code_challenge_method: 'plain',
  });
  const token = await new Promise((resolve, reject) => {
    const server = createServer(async (req, res) => {
      ...
      try { resolve(await exchange(code)); } catch (e) { reject(e); }
    });
...
async function gscFetch(token, endpoint, body) {
  ...
  if (res.status === 403) throw new Error('Access denied: the authorised account is not a user of this Search Console property, or the API is not enabled.');
  if (res.status === 404) throw new Error(`Property not found: ${SITE}. Check GSC_SITE (e.g. sc-domain:labs.techadyant.com or a full URL-prefix property).`);
...
async function main() {
  ...
  const [queries, pages, agg] = await Promise.all([
    gscFetch(token, 'searchAnalytics/query', { ...q, dimensions: ['query'], rowLimit: 25 }),
    gscFetch(token, 'searchAnalytics/query', { ...q, dimensions: ['page'], rowLimit: 25 }),
    gscFetch(token, 'searchAnalytics/query', { ...q, rowLimit: 1 }),
  ]);
  ...
  const md = [
    ...
    '',
    '## Top queries',
    mdTable(['Query', 'Clicks', 'Impressions', 'CTR', 'Position'], qRows),
    '',
    '## Top pages',
    mdTable(['Page', 'Clicks', 'Impressions', 'CTR', 'Position'], pRows),
    '',
